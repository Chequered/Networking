...
public class ServerNewPanel : MonoBehaviour {
    ...
    {
        NetworkManager.StartLobby(m_serverName.text, m_serverDescription.text, m_serverPassword.text);
        m_teamsCanvasGroup.alpha = 1;
        m_teamsCanvasGroup.interactable = true;
        m_teamsCanvasGroup.blocksRaycasts = true;
    ...
    [RPC]
    public void JoinNewSlot(int teamID, int colorID, string playerName, int oldTeam = 0, int oldTeamID = 0, int oldButtonIndex = 0)
    {
        Team team = TeamData.TeamColorByID(teamID);
        if(Network.isServer)
        {
